...
def encode(seq,vocabb):
    tok = tokenization.FullTokenizer(vocabb)
    vocab = tok.vocab
    tmp = []
    for w in seq:
        if w in vocab:
...
def _eval(sess, model):
    ...
    if best_auc < test_gauc:
        best_auc = test_gauc
        model.save(sess, 'save_path_change_new_split_61days/ckpt')
...
with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
    ...
    for _ in range(10):
        ...
        for _, uij in DataInput(train_set, train_batch_size):
            loss, logist = model.train(sess, uij, lr)
#             print(logist)
#             print('='*20)
            loss_sum += loss
