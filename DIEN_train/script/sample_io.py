...
class SampleIO(object):
    def __init__(self,
        ...
        self.train_data = DataIterator(train_file, uid_voc, mid_voc, cat_voc, item_info, reviews_info, batch_size, maxlen, shuffle_each_epoch=False)
        self.test_data = DataIterator(test_file, uid_voc, mid_voc, cat_voc, item_info, reviews_info, batch_size, maxlen)
        self.n_uid, self.n_mid, self.n_cat = self.train_data.get_n() # self.train_data.get_n()
    ...
    def _next_train(self):
        try:
            src, tgt = self.train_data.__next__()
    ...
    def _next_test(self):
        try:
            src, tgt = self.test_data.__next__()
        except StopIteration:
            self.src = self.tgt = None
    ...
    def _py_func(self, fn, sparse_cnt=7):
        ...
        for i in range(sparse_cnt):
            sparse_tensors.append(xdl.SparseTensor(datas[3 * i], datas[3 * i + 1], datas[3 * i + 2]))
    ...
    def prepare_data(self, input, target, maxlen=None, return_neg=False):
        ...
        neg_his_values = np.ones([n_samples * maxlen_x * neg_samples], np.float32)

        id_seg = np.array([i + 1 for i in range(n_samples)], dtype=np.int32)
        his_seg = np.array([i + 1 for i in range(n_samples * maxlen_x)], dtype=np.int32)
        neg_his_seg = np.array([i + 1 for i in range(n_samples * maxlen_x * neg_samples)], dtype=np.int32)
        ...
        results.extend([mid_mask, np.array(target, dtype=np.float32), np.array(lengths_x, dtype=np.int32)])
        # for split
