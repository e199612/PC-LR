...
test_file = os.path.join(get_data_prefix(), "test_back_0315.pkl")
...
def test(train_file=train_file,
    ...
    last_num = 0
    rank = []
    for i in range(0,len(pro_all),99):
        knn = knn_table[last_hist[last_num]]
        last_num += 1
        pro = pro_all[i:i+99]
    ...
    user = [i[1] for i in seq]
    hist = [i[4] for i in seq]
    ...
    with open('change_dien_rank_' + test_file[-12:],'wb') as d:
        pkl.dump(results,d)
