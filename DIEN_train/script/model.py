...
def eval_model(sess, test_ops):
    ...
    if best_auc < test_auc:
        best_auc = test_auc
    return pro_all, test_auc, loss_sum, accuracy_sum, aux_loss_sum
...
class Model(object):
    ...
    def auxiliary_net(self, in_, stag='auxiliary_net', reuse=tf.AUTO_REUSE):
        with tf.variable_scope("aux", reuse=reuse):
            ...
            dnn1 = tf.layers.dense(in_, 100, activation=None, kernel_initializer=get_tf_initializer(),name='f1' + stag, reuse=tf.AUTO_REUSE)
            dnn1 = tf.nn.sigmoid(dnn1)
            dnn2 = tf.layers.dense(dnn1, 50, activation=None, kernel_initializer=get_tf_initializer(),name='f2' + stag, reuse=tf.AUTO_REUSE)
            dnn2 = tf.nn.sigmoid(dnn2)
            dnn3 = tf.layers.dense(dnn2, 2, activation=None, kernel_initializer=get_tf_initializer(),name='f3' + stag, reuse=tf.AUTO_REUSE)
    ...
    def build_final_net(self, EMBEDDING_DIM, sample_io, is_train=True):
        ...
        if is_train:
            ...
            train_ops = tf_train_model(*self.xdl_embedding(datas, EMBEDDING_DIM, *sample_io.get_n()))
            return train_ops
        else:
            datas = sample_io.next_test()
            test_ops = tf_test_model(*self.xdl_embedding(datas, EMBEDDING_DIM, *sample_io.get_n()))
            return test_ops
...
class Model_DIEN(Model):
    def __init__(self, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE, use_negsampling=True):
        super(Model_DIEN, self).__init__(EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE, use_negsampling)
    ...
    def build_final_net(self, EMBEDDING_DIM, sample_io, is_train=True):
        ...
        def tf_test_model(*inputs):
            ...
            return test_ops[0], test_ops[1:]
        if is_train: # true
            datas = sample_io.next_train() # return from def _py_func
            train_ops = tf_train_model(*self.xdl_embedding(datas, EMBEDDING_DIM, *sample_io.get_n()))
            return train_ops
        else:
            datas = sample_io.next_test()
...
class Model_DIN(Model):
    ...
    def xdl_embedding(self, datas, EMBEDDING_DIM, n_uid, n_mid, n_cat):
        results = []
        uid_emb = xdl.embedding("uid_embedding", datas[0], get_xdl_initializer(),EMBEDDING_DIM, n_uid, 'sum') # datas[0] = uid
        results.append(uid_emb)
        for i in range(3):
            mid_emb = xdl.embedding("mid_embedding", datas[i * 2 + 1], get_xdl_initializer(),
                                    EMBEDDING_DIM, n_mid, 'sum')
